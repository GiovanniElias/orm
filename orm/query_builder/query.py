...
def get_create_stmt(table_name: str, column_definitions: list[Column]):
    ...
    return CREATE_TABLE_BASE_STMT.format(
        ...
        column_definitions=column_definitions_string,
    )
...
def get_insert_stmt(table_name: str, columns: list[Column], values: list[Any]):
    ...
    values_string = ', '.join(
        [
            f"'{val}'" if isinstance(val, (str, date, datetime)) else str(
                val,
            ) for val in values
        ],
    )
...
def get_update_stmt(
    table_name: str, set_columns: list[str], conditions: list[Column] = None,
):
